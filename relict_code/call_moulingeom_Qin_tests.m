...
parameters.H                   = 1497;%fliplr([669 820 947 1058 1159 1252 1339 1420 1497 1569]);  %ice thicknesses to use for the model
...
parameters.Qinrange            = 0.5 : 0.5 : 3;     % of the form:  Qin     = Qin*Qinrange + Qinbase;
    nqinrange = length(parameters.Qinrange);
parameters.Qinbase             = (1 : 1 : 6)';      % of the form:  Qin     = Qin*Qinrange + Qinbase;
...
%     disp(['Working on run ', num2str(ii), ' of ' num2str(maxlengthofseries), '...'])
    fprintf('Working on run %d of %d:   Qin = %1.1f range + %1.1f base   : ',ii,maxlengthofseries,parameters.Qinrange(ii),parameters.Qinbase(ii));
    ...
    %clear time
    diditgo = (length(time.S) == length(time.t));
    switch diditgo
        case 1, fprintf('Success!\n')
        otherwise, fprintf('Moulin overflow, fail\n')
    end
    success(ii) = diditgo;
end
...
%%
figure(5); clf; 
pch = imagesc(unique(parameters.Qinbase),parameters.Qinrange(1:nqinrange),double(reshape(success,nqinrange,nqinbase))); 
tcc = colorbar; tcc.YLabel.String = 'Model run successful for 10 days?';
set(gca,'ydir','normal')
tcc.Ticks = [0 1];
colormap([0.6 0.1 0.1;    0.1 0.7 1]);
% cbarf(double(reshape(success,nqinrange,nqinbase)),[0 0.5 0.75 2]);
xlabel('Qin base (m$^3$/sec)')
ylabel('Qin range (m$^3$/sec)')
title(sprintf('Ice thickness H=%1.0f m, L=%1.0f km',parameters.H(1),parameters.L(1)/1e3))
grid off
print('-dpng',sprintf('%s/Qins_H%dm.png','./',parameters.H(1)))

%%
cd(workingdirectory)
plotQinpossibilities(parameters.H(1),parameters.Qinrange,parameters.Qinbase,success)
%%
   ...
   copyfile([workingdirectory, '/call_moulingeom_series_cosine_watersens.m'], [savelocation, '/', savefile, '_call_moulingeom_series_cosine_watersens.m']);
